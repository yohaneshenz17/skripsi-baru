...
class Proposal extends CI_Controller
{
    ...
    public function ajukan()
    {
        if ($this->input->post()) {
            ...
            // Cek apakah sudah ada proposal aktif sebelum memproses
            $existing = $this->db->get_where('proposal_mahasiswa', [
                'mahasiswa_id' => $mahasiswa_id,
                'status !=' => '2' // Jika statusnya bukan ditolak
            ])->row();
     ...
     */
    private function _send_notification_to_mahasiswa($mahasiswa_id, $proposal_id, $post_data)
    {
        try {
     ...
     */
    private function _send_notification_to_kaprodi($mahasiswa_id, $proposal_id, $post_data)
    {
        try {
            // Ambil data mahasiswa dan prodi
    ...
    /**
     * Template email untuk mahasiswa
     */
