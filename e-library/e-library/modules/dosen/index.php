...
$count_where = '';
if (!empty($search)) {
    $where = "WHERE nuptk LIKE '%$search%' OR nama LIKE '%$search%' OR program_studi LIKE '%$search%'";
    $count_where = $where;
}

// Get total records
$count_query = "SELECT COUNT(*) as total FROM dosen $count_where";
$count_result = $conn->query($count_query);
$total_records = $count_result->fetch_assoc()['total'];
$total_pages = ceil($total_records / $records_per_page);

// Get data with pagination
$query = "SELECT * FROM dosen $where ORDER BY id DESC LIMIT $records_per_page OFFSET $offset";
...
<head>
    ...
    <title>Data Dosen - E-Library STK Yakobus</title>
...
<body>
    ...
    <nav class="sidebar">
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="<?= BASE_URL ?>modules/dosen/index.php">
    ...
    <main class="main-content">
        <div class="page-header d-flex justify-content-between align-items-center">
            <h1><i class="bi bi-person-badge me-2"></i>Data Dosen</h1>
            <div>
                ...
                <div class="row mb-3 align-items-center">
                    <div class="col-md-6">
                        <form method="GET" action="">
                            <div class="input-group search-box">
                                <input type="text" class="form-control" name="search" placeholder="Cari NUPTK, nama, atau program studi..." value="<?= htmlspecialchars($search) ?>">
                    ...
                    <div class="col-md-6 text-end">
                        <span class="pagination-info">
                            ...
                            Menampilkan <strong><?= number_format($showing_from) ?></strong> - <strong><?= number_format($showing_to) ?></strong> dari <strong><?= number_format($total_records) ?></strong> dosen
                        </span>
                    </div>
                </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= $no++ ?></td>
                                <td>
                                    <?php if (!empty($row['foto']) && file_exists(UPLOAD_DOSEN . $row['foto'])): ?>
                                        <img src="<?= BASE_URL ?>uploads/dosen/<?= $row['foto'] ?>" class="photo-thumb" alt="Foto">
                                    <?php else: ?>
                                        <div class="photo-thumb bg-info d-flex align-items-center justify-content-center text-white">
                                            <i class="bi bi-person"></i>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td><code><?= htmlspecialchars($row['nuptk']) ?></code></td>
                                ...
                                <td>
                                    ...
                                    <a href="delete.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus dosen ini?')">
                            ...
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">
                                    ...
                                    <?= !empty($search) ? 'Tidak ada hasil pencarian' : 'Tidak ada data dosen' ?>
                                </td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
