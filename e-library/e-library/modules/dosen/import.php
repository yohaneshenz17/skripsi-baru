        ...
        if ($ext !== 'csv') {
            ...
            if (($handle = fopen($file['tmp_name'], "r")) !== FALSE) {
                ...
                while (($row = fgetcsv($handle, 1000, $delimiter)) !== FALSE) {
                    ...
                    $nuptk = isset($row[0]) ? trim(str_replace("\xEF\xBB\xBF", '', $row[0])) : '';
                    $nama = isset($row[1]) ? trim($row[1]) : '';
                    $program_studi = isset($row[2]) ? trim($row[2]) : '';
                    $no_hp = isset($row[3]) ? trim($row[3]) : '';
                    
                    // Skip empty rows
                    if (empty($nuptk) && empty($nama)) continue;
                    
                    // Validate required fields
                    if (empty($nuptk) || empty($nama) || empty($program_studi)) {
                        $results[] = array(
                            'status' => 'failed',
                            'nuptk' => $nuptk ?: '(kosong)',
                            'nama' => $nama ?: '(kosong)',
                            'message' => 'Data tidak lengkap (NUPTK, Nama, atau Program Studi kosong)'
                        );
                        $failed_count++;
                        continue;
                    }
                    ...
                    $valid_prodi = ['S1 Pendidikan Keagamaan Katolik', 'S1 Pendidikan Guru Sekolah Dasar', 'Profesi Pendidikan Guru PAK'];
                    if (!in_array($program_studi, $valid_prodi)) {
                        $results[] = array(
                            'status' => 'failed',
                            'nuptk' => $nuptk,
                            'nama' => $nama,
                            'message' => 'Program Studi tidak valid. Gunakan: ' . implode(', ', $valid_prodi)
                        );
                        $failed_count++;
                        continue;
                    }
                    ...
                    if ($stmt->fetch()) {
                        ...
                        $results[] = array(
                            ...
                            'nuptk' => $nuptk,
                            'nama' => $nama,
                            'message' => 'NUPTK sudah ada di database'
                        );
                        $duplicate_count++;
                        continue;
                    }
                    ...
                    // Insert to database
                    $query = "INSERT INTO dosen (nuptk, nama, program_studi, no_hp) 
                              VALUES (?, ?, ?, ?)";
                    $stmt = $conn->prepare($query);
                    $stmt->bind_param("ssss", $nuptk, $nama, $program_studi, $no_hp);
...
<head>
    ...
    <title>Import Dosen CSV - E-Library STK Yakobus</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="bi bi-filetype-csv me-2"></i>Import Dosen dari CSV</h1>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                <p>Download template CSV untuk diisi dengan data dosen.</p>
                                <a href="template_csv.php" class="btn btn-primary" download>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <ol>
                                    ...
                                    <li>Isi data dosen (hapus baris contoh)</li>
                                ...
                                <div class="alert alert-info mb-3">
                                    ...
                                    <ol class="mb-0 mt-2">
                                        <li><strong>NUPTK</strong> (wajib, unik, 16 digit)</li>
                                        <li><strong>Nama Lengkap</strong> (wajib)</li>
                                        <li><strong>Program Studi</strong> (wajib, pilih dari list)</li>
                                        <li><strong>No. HP</strong> (opsional)</li>
                                    </ol>
                                </div>
                                
                                <div class="alert alert-warning mb-0">
                                    <strong>Program Studi Valid:</strong>
                                    <ul class="mb-0 mt-2">
                            ...
                            <table class="table table-sm table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><code><?= htmlspecialchars($r['nuptk']) ?></code></td>
