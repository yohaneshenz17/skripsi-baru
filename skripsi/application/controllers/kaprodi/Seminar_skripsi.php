...
class Seminar_skripsi extends CI_Controller {
    ...
    public function index() {
        ...
        // Ambil data proposal yang sudah dalam tahap seminar_skripsi
        ...
        $this->db->where('proposal_mahasiswa.workflow_status', 'seminar_skripsi');
        $this->db->order_by('proposal_mahasiswa.created_at', 'DESC');
        
        $data['seminar_skripsi'] = $this->db->get()->result();
        
        $this->load->view('kaprodi/seminar_skripsi', $data);
    }

    public function detail($proposal_id) {
        $data['title'] = 'Detail Seminar Skripsi';
        ...
        $this->load->view('kaprodi/seminar_skripsi_detail', $data);
    }
    ...
    public function validasi() {
        ...
        $update_data = [
            'status_seminar_skripsi' => ($status == 'approve') ? '1' : '2',
            'komentar_seminar_skripsi' => $komentar,
            'tanggal_review_seminar_skripsi' => date('Y-m-d H:i:s')
        ];
        
        if ($status == 'approve') {
            $update_data['dosen_penguji2_id'] = $dosen_penguji2_id;
            $update_data['tanggal_seminar_skripsi'] = $tanggal_seminar;
            $update_data['workflow_status'] = 'publikasi';
        }
        
        $this->db->where('id', $proposal_id)->update('proposal_mahasiswa', $update_data);
        
        $message = ($status == 'approve') ? 'Seminar skripsi disetujui!' : 'Seminar skripsi ditolak!';
